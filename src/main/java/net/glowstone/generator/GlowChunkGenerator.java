...
public abstract class GlowChunkGenerator extends ChunkGenerator {
    ...
    protected byte[] start(int fill) {
        ...
        byte[] data = new byte[GlowChunk.DEPTH * GlowChunk.WIDTH * GlowChunk.HEIGHT];
    ...
    protected void set(byte[] data, int x, int y, int z, int id) {
        ...
        if (x < 0 || y < 0 || z < 0 || x >= GlowChunk.DEPTH || y >= GlowChunk.HEIGHT || z >= GlowChunk.WIDTH) {
            return;
        }
        data[(x * GlowChunk.DEPTH + z) * GlowChunk.HEIGHT + y] = (byte) id;
    }
    ...
    protected int get(byte[] data, int x, int y, int z) {
        ...
        return data[(x * GlowChunk.DEPTH + z) * GlowChunk.HEIGHT + y];
    }
