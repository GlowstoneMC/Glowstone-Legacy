...
public class BlockLadder extends BlockType {
    ...
    public void placeBlock(GlowPlayer player, GlowBlockState state, BlockFace face, ItemStack holding, Vector clickedLoc) {
        ...
        if (data instanceof Ladder) {
            ...
            // complain?
            GlowServer.logger.warning("Placing Ladder: MaterialData was of wrong type");
        }
    }

    private boolean isTargetOccluding(GlowBlockState state, BlockFace face) {
        return state.getBlock().getRelative(face).getType().isOccluding();
    }
}
